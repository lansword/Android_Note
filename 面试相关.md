...
3. **Activity 的四种启动模式**
   ...
   -  **Intent 标志**
     ...
     -  **Intent.FLAG_ACTIVITY_NEW_TASK**：单独使用没有效果，调用startActivityForResult时也不能使用。搭配其他Intent标志一起使用时的作用是，先在系统中查找是否存在属性值`taskAffinity`等于它自己的任务栈存在，如果不存在其他标志则不起作用，系统将会另外创建一个task，然后将新的activity压进去；如果存在，则进一步看其他标志的作用。
       - 例如：Intent.FLAG_ACTIVITY_NEW_TASK和Intent.FLAG_ACTIVITY_CLEAR_TASK一起使用时，正常情况下可以用来清除app内所有的activity，跳转到新建的登录页面，这是因为正常情况下我们不会去指定taskAffinity，app内所有的activity都在同一个task内，如果我们单独指定被启动的activity(这里为登录页面)的taskAffinity，那结果就会是新建的登录页activity会放入系统新创建的另外一个task，从而无法达到我们的需求。
