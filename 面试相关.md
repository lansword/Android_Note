...
3. **Activity 的四种启动模式**
   ...
   - [Android 启动模式FLAG_ACTIVITY_CLEAR_TOP - 简书 (jianshu.com)](https://www.jianshu.com/p/cb248934c27c)
   ...
   -  Intent 标志
     ...
     - **Intent.FLAG_ACTIVITY_NEW_TASK**：单独使用没有效果，调用startActivityForResult时也不能使用;
     - **Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP**:这两者搭配使用时，并且目标activity的launchMode为standard时，会先在task栈内寻找目标activity是否存在，如果不存在则正常启动，如果存在则会*清除它上面的activity实例，自己会先销毁再重建*。
     - **Intent.FLAG_ACTIVITY_CLEAR_TASK:**搭配Intent.FLAG_ACTIVITY_NEW_TASK一起使用，则会将task内所有activity清除（不管目标activity是否已经存在），并创建目标activity，也就是说此时栈内只剩目标单独一个activity。注意，单独使用时，如果目标activity了启动模式为singleTask时也会有同样的效果。
